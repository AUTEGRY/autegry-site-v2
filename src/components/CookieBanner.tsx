...
import { hasGivenCookieConsent, setCookieConsent } from '@/utils/cookieConsent';
...
const CookieBanner = () => {
  ...
  // For development: To reset cookie consent, run this in browser console:
  // localStorage.removeItem('autegry_cookie_consent'); location.reload();

  useEffect(() => {
    // Check if user has already given consent
    const alreadyConsented = hasGivenCookieConsent();
    console.log('Cookie consent check:', alreadyConsented);

    if (alreadyConsented) {
      console.log('User has already given cookie consent, not showing banner');
      setIsVisible(false);
      return;
    }

    console.log('No cookie consent found, will show banner in 5 seconds');
    // If no consent given, show banner after 5 seconds
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-background shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-50 p-4 animate-slide-up">
      ...
      <div className="max-w-md mx-auto flex flex-col items-start text-left gap-4 md:hidden">
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          <Button onClick={handleNecessaryOnly} className="w-full">
      ...
      <div className="max-w-6xl mx-auto hidden md:flex items-start gap-4 px-2">
        ...
        <div className="flex flex-col gap-3 flex-shrink-0 w-64 ml-2">
          ...
          <Button onClick={handleNecessaryOnly} className="w-full text-sm px-8 py-3">
            {t('cookieBanner.necessary')}
          </Button>
        </div>
      </div>
